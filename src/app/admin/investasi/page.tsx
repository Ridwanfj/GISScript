  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/admin/investasi?page=${page}&limit=${limit}&search=${encodeURIComponent(debouncedSearch)}`,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          <h1 className="text-2xl font-semibold tracking-tight text-gray-900">Proyek Investasi</h1>
          <p className="mt-1 text-sm text-gray-500">Kelola data koordinat menengah dan besar</p>
        </div>
        <Link
      ...
      <div className="relative max-w-md">
        ...
        <input
          type="text"
          placeholder="Cari nama proyek, sektor, atau alamat..."
          value={search}
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr className="text-left text-xs font-medium uppercase tracking-wider text-gray-500">
                ...
                <th className="px-4 py-3">Jenis / Sektor</th>
                <th className="px-4 py-3">Skala Usaha</th>
                <th className="px-4 py-3">Nilai Investasi</th>
                ...
                data.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3">
                      <div className="font-medium text-gray-900">{item.nama_proyek}</div>
                      <div className="text-xs text-gray-500 mt-0.5 line-clamp-1">
                        {item['Alamat Lengkap']}
                      </div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="text-gray-900">{item.Uraian_Jenis_Proyek}</div>
                      <div className="text-xs text-gray-500 mt-0.5">{item.Sektor}</div>
                    </td>
                    <td className="px-4 py-3">
          ...
          <div className="w-full max-w-md rounded-lg bg-white border border-gray-200 shadow-lg p-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                <h3 className="text-base font-semibold text-gray-900">Hapus Proyek?</h3>
                <p className="mt-1 text-sm text-gray-500">
                  Data <span className="font-medium text-gray-900">{itemToDelete?.nama_proyek}</span> akan dihapus permanen.
                </p>
              </div>
            </div>
